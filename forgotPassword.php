...
if(isset($_POST['submit'])){
    ...
    if($userType == "ClassTeacher"){
        $query = "SELECT * FROM tblclassteacher WHERE emailAddress = '$identifier'";
    } else {
        $query = "SELECT * FROM tblstudents WHERE admissionNumber = '$identifier'";
    }
    ...
    if($result->num_rows > 0){
        ...
        $headers = "From: noreply@attendancesystem.com";
        
        // Send email
        if(mail($to, $subject, $message, $headers)){
            echo "<div class='alert alert-success' role='alert'>
                Password reset request has been sent to administrator. You will be contacted soon.
                </div>";
        } else {
            echo "<div class='alert alert-danger' role='alert'>
                Failed to send reset request. Please try again later.
                </div>";
        }
    } else {
        echo "<div class='alert alert-danger' role='alert'>
